...
Run `npm version <major|minor|patch>` from clean `main` to prepare a semantic version. npm checks that the worktree is clean, and Hookrelay's `preversion` guard requires `main`, verifies that every tracked D1 migration is recorded in production, and runs typechecking and tests. Migration verification fails closed when credentials, configuration, local migration files or the production ledger are unavailable or inconsistent. It requires `CLOUDFLARE_ACCOUNT_ID` and `CLOUDFLARE_API_TOKEN` with D1 Read permission; apply pending migrations with `pnpm exec wrangler d1 migrations apply EVENTS_DB --remote` before releasing. npm then creates the version commit and annotated `v<version>` tag. The `postversion` hook atomically pushes `main` and that tag to `origin`, so GitHub updates both refs or neither.
