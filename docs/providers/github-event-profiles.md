...
The selected profile names and repository are saved under `subs[].setup.github` in local `routes.jsonc`. They are setup metadata and are not copied into runtime provider configuration. `pnpm sync` compiles shared profile rules, or an explicit subscription override, into the selected authority's runtime filter configuration. `pnpm github:events` applies only that subscription's `filter` field before changing GitHub, preserving other online policy. It requires the sender secret mirrored in `.dev.vars` because GitHub's general webhook update clears an omitted secret; it resends that secret and the unchanged hook config through stdin while replacing the event array. Use `pnpm github:events` after changing profile metadata so the provider filter and upstream webhook remain aligned. `manual` changes the provider filter but leaves GitHub's remote selection unchanged.
